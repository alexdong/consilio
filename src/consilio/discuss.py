...
def edit_round(topic: Topic, round_num: int) -> None:
    ...
    files = [
        topic.round_input_file(round_num),
        topic.round_response_file(round_num)
    ]
...
def handle_discuss_command(edit: Optional[int], round: Optional[int]) -> None:
    ...
    click.echo("(Answer questions from the previous round of discussions, or specify a particular area you'd like to focus on next.)")
